...
# Bundled Rust-native servers (docs/architecture.md#bundled-rust-native-
# servers, e.g. lsp-json-lsp, lsp-css-lsp) must live next to `lsp` itself —
# registry.rs resolves each one relative to the running executable's own
# directory, not $PATH, so every lsp-*-lsp in the archive has to land in
# the same $INSTALL_DIR.
for bundled in "$tmp"/lsp-*-lsp; do
  [ -f "$bundled" ] || continue
  mv "$bundled" "$INSTALL_DIR/$(basename "$bundled")"
  chmod +x "$INSTALL_DIR/$(basename "$bundled")"
done
