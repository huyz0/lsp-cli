...
- **LSP JSON-RPC client** (`src/lsp_client.rs`): hand-rolled Content-Length
  ...
  matched via a typed `RpcError` enum rather than string-matching; enforces
...
(`LspClient::sync_document`) — some servers (typescript-language-server)
...
evicted only by `lsp server stop`, an idle timeout, or a detected crash.
...
tuned against multiple simultaneously-warm servers competing for CPU. A
...
`java` on `PATH` (this tool doesn't install a JDK itself — much bigger,
more opinionated a dependency than any other managed server — so `lsp
...
default — important for scripted/agent callers, where a silently-wrong
