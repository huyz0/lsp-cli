...
| Zig | `zls` (auto-installable: fetches the latest GitHub Release tar.xz — a bare binary at the archive root, no wrapping directory) | ✅ verified live: real `outline` against a `pub fn main` returned it with the correct `fn main() void` signature. No dedicated `tests/*.rs` file yet. |
...
| C# | `csharp-ls` (via `dotnet tool install --tool-path`) | ✅ verified live once a .NET SDK was made available: `outline` against a two-class file returned the correct `CsTest` namespace → `Greeter` (field/constructor/method) and `Program` (Main) tree; `definition` on a constructor call resolved correctly; `doc` (hover) returned the correct method signature. No dedicated `tests/*.rs` file yet. |
